...
public class CustomToggleButtonRenderer : IToggleButtonRenderer
{
    ...
    {
        // Draw border logic
        Color borderColor = isActive ? Color.DarkGreen : Color.DarkGray;
    ...
    public void DrawBackground(Graphics graphics, Rectangle bounds, bool isActive, bool isHovered)
    {
        ...
        Color backgroundColor = isActive ? Color.LightGreen : Color.LightGray;
        if (isHovered)
        {
            backgroundColor = isActive ? Color.Green : Color.Gray;
        }
...
### Step 2: Implement Rounded Corners Renderer

Example of a custom renderer with rounded corners:
...
Once you've created your custom renderer, assign it to the Toggle Button:
...
public Form1()
{
    ...
    CustomToggleButtonRenderer renderer = new CustomToggleButtonRenderer();
    
    // Assign to toggle button
    toggleButton1.Renderer = renderer;
}
```

### Method 2: Assignment After Creation

```csharp
private void ApplyCustomRenderer()
{
    RoundedCornerToggleButtonRenderer renderer = new RoundedCornerToggleButtonRenderer();
    this.toggleButton1.Renderer = renderer;
}
...
### Method 3: Dynamic Renderer Switching

```csharp
private IToggleButtonRenderer _defaultRenderer;
private IToggleButtonRenderer _customRenderer;

public Form1()
{
    InitializeComponent();
    _customRenderer = new RoundedCornerToggleButtonRenderer();
}

private void UseDefaultRenderer()
{
    this.toggleButton1.Renderer = _defaultRenderer;
}

private void UseCustomRenderer()
{
    this.toggleButton1.Renderer = _customRenderer;
}
```
...
### Customization 1: Change Border Style

```csharp
public void DrawBorder(Graphics graphics, Rectangle bounds, bool isActive, bool isHovered)
{
    Color borderColor = isActive ? Color.Blue : Color.Gray;
    float penWidth = isHovered ? 3 : 2;
...
### Customization 2: Increase Font Size
...
### Customization 3: Add Opacity Effects
...
public void DrawBackground(Graphics graphics, Rectangle bounds, bool isActive, bool isHovered)
{
    ...
    Color backgroundColor = isActive ? 
        Color.FromArgb(alpha, 34, 177, 76) : 
        Color.FromArgb(alpha, 150, 150, 150);
